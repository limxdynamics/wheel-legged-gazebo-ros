/**
 * @file LeggedHWSim.cpp
 *
 * @brief This file defines the LeggedHWSim class, which is responsible for simulating a legged robot.
 *
...
static limxsdk::WheelLeggedSim *wl;
...
namespace legged_gazebo
{
    ...
    {
        // Initialize the WheelLeggedSim instance and connect to the robot at IP address 127.0.0.1
        wl = limxsdk::WheelLeggedSim::getInstance();
        wl->init("127.0.0.1");
    ...
    int LeggedHWSim::parseJointIndex(const std::string& jointName) {
        int leg_index = 0;
        int joint_index = 0;

        // Determine the leg index based on the joint name
        // RF: Right Front leg, LF: Left Front leg, RH: Right Hind leg, LH: Left Hind leg
        if (jointName.find("RF") != std::string::npos)
        {
            leg_index = 2;  // Right Front leg
        }
        else if (jointName.find("LF") != std::string::npos)
        {
            leg_index = 0;  // Left Front leg
        }
        else if (jointName.find("RH") != std::string::npos)
        {
            leg_index = 3;  // Right Hind leg
        }
        else if (jointName.find("LH") != std::string::npos)
        {
            leg_index = 1;  // Left Hind leg
        }
        else
        {
            return -1;  // Unknown leg name, return -1
        }

        // Determine the joint index based on the joint name
        // HAA: Hip Abduction/Adduction, HFE: Hip Flexion/Extension, KFE: Knee Flexion/Extension, WHL: Wheel joint
        if (jointName.find("HAA") != std::string::npos)
        {
            joint_index = 0;  // Hip Abduction/Adduction joint
        }
        else if (jointName.find("HFE") != std::string::npos)
        {
            joint_index = 1;  // Hip Flexion/Extension joint
        }
        else if (jointName.find("KFE") != std::string::npos)
        {
            joint_index = 2;  // Knee Flexion/Extension joint
        }
        else if (jointName.find("WHL") != std::string::npos)
        {
            joint_index = 3;  // Wheel joint
        }
        else
        {
            return -1;  // Unknown joint name, return -1
        }

        // Calculate the combined index of leg and joint taking into account a total of 4 joints per leg
        return (leg_index * 4 + joint_index);
    }

} // namespace legged_gazebo

// Export the LeggedHWSim class as a plugin for gazebo_ros_control::RobotHWSim
PLUGINLIB_EXPORT_CLASS(legged_gazebo::LeggedHWSim, gazebo_ros_control::RobotHWSim)
